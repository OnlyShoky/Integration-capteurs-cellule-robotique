...
def callback_logical_camera(data):
    if(len(data.models) > 0) :
        for index in range(0, 8):
            if(data.models[0].type == bins[index]):
                rospy.loginfo("Object detecte " + str(data.models[0].type) + " vers bac"+str(index+1))
                rospy.loginfo("Pos " + str(data.models[0].pose.position) )



def update_back_bin1(data):
    if(len(data.models) > 0) :
        bins[0] = data.models[0].type
        #bins.insert(0,data.models[0].type)

def update_back_bin2(data):
    if(len(data.models) > 0) :
        bins[1] = data.models[0].type
        #bins.insert(0,data.models[0].type)

def update_back_bin3(data):
    if(len(data.models) > 0) :
        bins[2] = data.models[0].type
        #bins.insert(0,data.models[0].type)
def update_back_bin4(data):
    if(len(data.models) > 0) :
        bins[3] = data.models[0].type
        #bins.insert(0,data.models[0].type)

def update_back_bin5(data):
    if(len(data.models) > 0) :
        bins[4] = data.models[0].type
        #bins.insert(0,data.models[0].type)

def update_back_bin6(data):
    if(len(data.models) > 0) :
        bins[5] = data.models[0].type
        #bins.insert(0,data.models[0].type)

def update_back_bin7(data):
    if(len(data.models) > 0) :
        bins[6] = data.models[0].type
        #bins.insert(0,data.models[0].type)

def update_back_bin8(data):
    if(len(data.models) > 0) :
        bins[7] = data.models[0].type
        #bins.insert(0,data.models[0].type)
...
bins = ["nan", "nan", "nan", "nan", "nan", "nan", "nan", "nan"]
...
def listener():
    ...
    #logial_camera = rospy.Subscriber("/ariac/logical_camera_10", LogicalCameraImage, callback_logical_camera)

    logial_camera_bin1 = rospy.Subscriber("/ariac/logical_camera_over_belt", LogicalCameraImage, callback_logical_camera)


    logial_camera_bin1 = rospy.Subscriber("/ariac/logical_camera_bin1", LogicalCameraImage, update_back_bin1)
    logial_camera_bin2 = rospy.Subscriber("/ariac/logical_camera_bin2", LogicalCameraImage, update_back_bin2)
    logial_camera_bin3 = rospy.Subscriber("/ariac/logical_camera_bin3", LogicalCameraImage, update_back_bin3)
    logial_camera_bin4 = rospy.Subscriber("/ariac/logical_camera_bin4", LogicalCameraImage, update_back_bin4)

    logial_camera_bin5 = rospy.Subscriber("/ariac/logical_camera_bin5", LogicalCameraImage, update_back_bin5)
    logial_camera_bin6 = rospy.Subscriber("/ariac/logical_camera_bin6", LogicalCameraImage, update_back_bin6)
    logial_camera_bin7 = rospy.Subscriber("/ariac/logical_camera_bin7", LogicalCameraImage, update_back_bin7)
    logial_camera_bin8 = rospy.Subscriber("/ariac/logical_camera_bin8", LogicalCameraImage, update_back_bin8)

    rospy.loginfo(bins)


    #break_bream = rospy.Subscriber("/ariac/break_beam_over_belt", Proximity, callback_break_beam)
